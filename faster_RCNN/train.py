...
def main():
    ...
    train_data_set = sphaera(cfg.data_root_dir, 'train', '2017', data_transform["train"], prefix="", batch_size=8, include_class=[0,1,2,3,4])
    ...
    val_data_set = sphaera(cfg.data_root_dir, 'val', '2017', data_transform["val"], prefix="", batch_size=8, include_class=[0,1,2,3,4])
    ...
    for epoch in range(cfg.start_epoch, cfg.num_epochs):
        ...
        _, train_mAP = evaluate(model, train_data_loader, device=device, mAP_list=train_mAP_list, single_cls=cfg.single_cls)
        print("------>Starting validation data valid")
        _, mAP = evaluate(model, val_data_set_loader, device=device, mAP_list=val_mAP, single_cls=cfg.single_cls)
